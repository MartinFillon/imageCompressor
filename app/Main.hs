...
calcKmeans ::
    ...
    Bool ->
    ([ImageData], [(Int, Color)])
calcKmeans dt st _ n True = (dt, zip [1 ..] $ map colorFrom $ computeClusterMeans n st dt)
calcKmeans dt st c n False = calcKmeans ndata mn c n (checkConvergence c $ zip st mn)
  where
